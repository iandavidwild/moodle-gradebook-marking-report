...
/**
 * File in which the marking_report class is defined.
 * @package gradebook
 */
...
/**
 * Class providing an API for the marking report building and displaying.
 * @uses grade_report
 * @package gradebook
 */
class grade_report_marking extends grade_report {
    /**
     * The final grades.
     * @var array $grades
     */
...
//// SQL-RELATED
    ...
    public function __construct($courseid, $gpr, $context, $page=null, $sortitemid=null) {
        ...
        if ($collapsed = get_user_preferences('grade_report_marking_collapsed_categories')) {
        ...
        $this->pbarurl = new moodle_url('/grade/report/marking/index.php', array('id' => $this->courseid, 'perpage' => $studentsperpage));
    ...
    /**
    * Shortcut function for printing the marking report toggles.
    ...
    */
    ...
    /**
     * Builds and returns the rows that will make up the left part of the marking report
    ...
    public function get_left_rows() {
        ...
        $colspan = 1;
        /* 21/08/2012 IDW - We aren't showing any extra fields so this bit of code is redundant...
        ...
        */
        ...
        // 20/08/2012 IDW - Don't show a link to the grade report. This is meant to be anonymous.
        /*
        if (has_capability('gradereport/'.$CFG->grade_profilereport.':view', $this->context)) {
        ...
        */
        ...
        foreach ($this->users as $userid => $user) {
            ...
            // IDW 30/7/12 only list username
            /*if ($showuserimage) {
                $usercell->text = $OUTPUT->user_picture($user);
            }*/

            $usercell->text .= $user->username;
            ...
            /* 20/08/2012 IDW Don't show a link to a user's grader report. This is meant to be anonymous.
    ...
    /**
     * Builds and returns the rows that will make up the right part of the marking report
     * @return array Array of html_table_row objects
     */
...
// Element is a grade_item
                    ...
                    // 21/08/2012 IDW - We don't want the header to link anywhere... 
                    $headerlink = $this->gtree->get_element_header($element, false, $this->get_pref('showactivityicons'), false);
        ...
        foreach ($this->users as $userid => $user) {
            ...
            // IDW 20/08/2012 pass the username to module.js as we don't want the marker to know exactly who they are marking
            //$jsarguments['users'][$userid] = fullname($user);
            
            $jsarguments['users'][$userid] = $user->username;
            ...
            foreach ($this->gtree->items as $itemid=>$unused) {
                ...
                // MDL-11274
                // Hide grades in the marking report if the current marking doesn't have 'moodle/grade:viewhidden'
                ...
                if ($item->needsupdate) {
                    ...
                    if ($item->needsupdate) {
                        ...
                        /* IDW 21/08/2012 - Don't show grade analysis icon. It's meant to be anonymous. 
        ...
        $module = array(
            'name'      => 'gradereport_marking',
            'fullpath'  => '/grade/report/marking/module.js',
            'requires'  => array('base', 'dom', 'event', 'event-mouseenter', 'event-key', 'io-base', 'json-parse', 'overlay')
        );
        $PAGE->requires->js_init_call('M.gradereport_marking.init_report', $jsarguments, false, $module);
        $PAGE->requires->strings_for_js(array('addfeedback','feedback', 'grade'), 'grades');
        $PAGE->requires->strings_for_js(array('ajaxchoosescale','ajaxclicktoclose','ajaxerror','ajaxfailedupdate', 'ajaxfieldchanged'), 'gradereport_marking');
    ...
    /**
     * Builds and return the header for the row of ranges, for the left part of the marking report.
     * @param array $rows The Array of rows for the left part of the report
     * @param int $colspan The number of columns this cell has to span
     * @return array Array of rows for the left part of the report
     */
    ...
    /**
     * Builds and return the headers for the rows of averages, for the left part of the marking report.
     * @param array $rows The Array of rows for the left part of the report
     * @param int $colspan The number of columns this cell has to span
    ...
    /**
     * Builds and return the row of icons when editing is on, for the right part of the marking report.
    ...
    /**
     * Builds and return the row of ranges for the right part of the marking report.
     * @param array $rows The Array of rows for the right part of the report
     * @return array Array of rows for the right part of the report
     */
    ...
    /**
     * Builds and return the row of averages for the right part of the marking report.
    ...
    /**
     * Return audit icon for given element
     *
     ...
     * @return string
     */
    protected function get_audit_icon($element, $gpr) {
    	global $CFG, $OUTPUT;
    
    	if (!has_capability('moodle/grade:manage', $this->context)) {
    		if ($element['type'] == 'grade' and has_capability('moodle/grade:edit', $this->context)) {
    			// ok - let them override grade
    		} else {
    			return '';
    		}
    	}
    
    	static $strgradeaudit = null;
    	if (is_null($strgradeaudit)) {
    		$strgradeaudit = get_string('grade_audit', 'gradereport_marking');
    	}
    	...
    	switch ($element['type']) {
    		case 'grade':
    			if (empty($object->id)) { // IDW 20/08/2012 Will need to have a think about this 'if'
    				$url = new moodle_url('/grade/report/marking/audit.php',
    ...
    /**
     * Return mark icon for given element
     *
     * @param array  $element An array representing an element in the grade_tree
     * @param object $gpr A grade_plugin_return object
     *
     * @return string
     */
    protected function get_mark_icon($element, $gpr) {
    	global $CFG, $OUTPUT;
    	...
    	static $strmarkgrade = null;
    	if (is_null($strmarkgrade)) {
    		$strmarkgrade = get_string('editgrade', 'grades');
    	}
    	...
    	switch ($element['type']) {
    		case 'grade':
    			if (empty($object->id)) { // IDW 20/08/2012 Open special 'marking' form
    				$url = new moodle_url('/grade/report/marking/blindmark.php',
    						array('courseid' => $this->courseid, 'itemid' => $object->itemid, 'userid' => $object->userid));
    			} else {
    				$url = new moodle_url('/grade/report/marking/blindmark.php',
    						array('courseid' => $this->courseid, 'id' => $object->id));
    	...
    	if ($url) {
    		return $OUTPUT->action_icon($gpr->add_url_params($url), new pix_icon('t/edit', $strmarkgrade));
     ...
     * with the icons needed for the marking report.
     *
     * @param array $object
     * @return string HTML
     */
    protected function get_icons($element) {
        ...
        // IDW 20/08/2012 - Show an 'Audit trail' icon but only for grades (i.e. not categories, etc)
        $gradeauditicon      = '';
        if ($element['type'] != 'categoryitem' && $element['type'] != 'courseitem') {
        	$gradeauditicon = $this->get_audit_icon($element, $this->gpr);
        }
        
        $markicon = '';

        if ($element['type'] != 'categoryitem' && $element['type'] != 'courseitem') {
            $markicon = $this->get_mark_icon($element, $this->gpr);
        }
        ...
        /* IDW 20/08/2012 - Don't show grade analysis icon on Marking report
        ...
        */
        return $OUTPUT->container($markicon.$editcalculationicon.$showhideicon.$lockunlockicon.$gradeauditicon, 'grade_icons');
    }
        ...
        switch ($action) {
            case 'switch_minus': // Add category to array of aggregatesonly
                if (!in_array($targetid, $collapsed['aggregatesonly'])) {
                    ...
                    set_user_preference('grade_report_marking_collapsed_categories', serialize($collapsed));
                }
                break;
