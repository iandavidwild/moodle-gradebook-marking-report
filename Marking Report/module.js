/**
 * Marking report namespace
 */
M.gradereport_marking = {
    /**
     * @param {Array} reports An array of instantiated report objects
     */
    reports : [],
    /**
     * @namespace M.gradereport_marking
    ...
    init_report : function(Y, id, cfg, items, users, feedback) {
        this.tooltip = this.tooltip || {
             ...
             *
             * @function M.gradereport_marking.tooltip.attach
             * @this M.gradereport_marking
            ...
            /**
             * Shows the tooltip: Callback from @see M.gradereport_marking.tooltip#attach
             *
             * @function M.gradereport_marking.tooltip.show
             * @this {M.gradereport_marking.tooltip}
             * @param {Event} e
             * @param {M.gradereport_marking.classes.report} report
             */
            show : function(e, report) {
                ...
                var content = '<div class="markingreportoverlay">';
                content += '<div class="name">'+properties.username+'</div><div class="itemname">'+properties.itemname+'</div>';
             ...
             * Hides the tooltip
             *
             * @function M.gradereport_marking.tooltip.hide
             * @this {M.gradereport_marking.tooltip}
             * @param {Event} e
             * @param {Y.Node} cell
             */
...
/**
 * Initialises the JavaScript for the gradebook marking report
 *
 * @class report
 * @constructor
 * @this {M.gradereport_marking}
 ...
 */
M.gradereport_marking.classes.report = function(Y, id, cfg, items, users, feedback) {
    this.Y = Y;
    ...
    this.table.all('tr').each(function(tr){
        ...
        if (tr.getAttribute('id').match(/^(fixed_)?user_(\d+)$/)) {
            ...
            tr.all('td.cell').each(function(cell){
                M.gradereport_marking.tooltip.attach(cell, this);
            }, this);
        }
    }, this);
    ...
    if (this.ajaxenabled) {
        this.ajax = new M.gradereport_marking.classes.ajax(this, cfg);
    }
};
/**
 * Extend the report class with the following methods and properties
 */
M.gradereport_marking.classes.report.prototype.table = null;           // YUI Node for the reports main table
M.gradereport_marking.classes.report.prototype.items = [];             // Array containing grade items
M.gradereport_marking.classes.report.prototype.users = [];             // Array containing user information
M.gradereport_marking.classes.report.prototype.feedback = [];          // Array containing feedback items
M.gradereport_marking.classes.report.prototype.ajaxenabled = false;    // True is AJAX is enabled for the report
M.gradereport_marking.classes.report.prototype.ajax = null;            // An instance of the ajax class or null
/**
 ...
 */
M.gradereport_marking.classes.report.prototype.table_highlight_row = function (e, tr) {
 ...
 * @param {Y.Node} cell
 */
M.gradereport_marking.classes.report.prototype.table_highlight_column = function(e, cell) {
    var column = 0;
 ...
 * @return {Object}
 */
M.gradereport_marking.classes.report.prototype.get_cell_info = function(arg) {
 ...
 * @return {Bool}
 */
M.gradereport_marking.classes.report.prototype.update_feedback = function(userid, itemid, newfeedback) {
 ...
 * @class ajax
 * @constructor
 * @this {M.gradereport_marking.ajax}
 * @param {M.gradereport_marking.classes.report} report
 * @param {Object} cfg
 */
M.gradereport_marking.classes.ajax = function(report, cfg) {
    this.report = report;
    ...
    if (!M.gradereport_marking.isediting) {
 ...
 */
M.gradereport_marking.classes.ajax.prototype.report = null;                  // A reference to the report class this object will use
M.gradereport_marking.classes.ajax.prototype.courseid = null;                // The id for the course being viewed
M.gradereport_marking.classes.ajax.prototype.feedbacktrunclength = null;     // The length to truncate feedback to
M.gradereport_marking.classes.ajax.prototype.studentsperpage = null;         // The number of students shown per page
M.gradereport_marking.classes.ajax.prototype.showquickfeedback = null;       // True if feedback editing should be shown
M.gradereport_marking.classes.ajax.prototype.current = null;                 // The field being currently editing
M.gradereport_marking.classes.ajax.prototype.pendingsubmissions = [];        // Array containing pending IO transactions
M.gradereport_marking.classes.ajax.prototype.scales = [];                    // An array of scales used in this report
/**
 * Makes a cell editable
 * @function
 * @this {M.gradereport_marking.classes.ajax}
 */
M.gradereport_marking.classes.ajax.prototype.make_editable = function(e) {
    var node = e;
    if (e.halt) {
        e.halt();
        node = e.target;
    ...
    switch (fieldtype) {
        case 'scale':
            this.current = new M.gradereport_marking.classes.scalefield(this.report, node);
            break;
        case 'text':
        default:
            this.current = new M.gradereport_marking.classes.textfield(this.report, node);
            break;
    }
 ...
 * @param {Event} e
 */
M.gradereport_marking.classes.ajax.prototype.keypress_enter = function(e) {
 ...
 */
M.gradereport_marking.classes.ajax.prototype.keypress_tab = function(e, ignoreshift) {
    var next = null;
    if (e.shiftKey) {
        if (ignoreshift) {
            return;
 ...
 * @this {M.gradereport_marking.classes.ajax}
 */
M.gradereport_marking.classes.ajax.prototype.keypress_arrows = function(e) {
    e.preventDefault();
    var next = null;
    switch (e.keyCode) {
        case 37:    // Left
 ...
 */
M.gradereport_marking.classes.ajax.prototype.process_editable_field = function(next) {
 ...
 * @return {Y.Node}
 */
M.gradereport_marking.classes.ajax.prototype.get_next_cell = function(cell) {
 ...
 * @return {Y.Node}
 */
M.gradereport_marking.classes.ajax.prototype.get_prev_cell = function(cell) {
    var n = cell || this.current.node;
 ...
 * @return {Y.Node}
 */
M.gradereport_marking.classes.ajax.prototype.get_above_cell = function(cell) {
    var n = cell || this.current.node;
 ...
 * @return {Y.Node}
 */
M.gradereport_marking.classes.ajax.prototype.get_below_cell = function(cell) {
    var n = cell || this.current.node;
 ...
 */
M.gradereport_marking.classes.ajax.prototype.submit = function(properties, values) {
    ...
    if (values.grade !== values.oldgrade) {
        this.pendingsubmissions.push({transaction:this.report.Y.io.queue(M.cfg.wwwroot+'/grade/report/marking/ajax_callbacks.php', {
            method : 'POST',
 ...
 * @param {Mixed} args
 */
M.gradereport_marking.classes.ajax.prototype.submission_outcome = function(tid, outcome, args) {
    // Parse the response as JSON
    try {
        outcome = this.report.Y.JSON.parse(outcome.responseText);
    } catch(e) {
        var message = M.str.gradereport_marking.ajaxfailedupdate;
        message.replace(/\[1\]/, args.type);
        message.replace(/\[2\]/, M.gradereport_marking.users[args.properties.userid]);
        this.display_submission_error(message, args.properties.cell);
        return;
    }

    // Quick reference for the marking report
    var i = null;
    // Check the outcome
            ...
            if (outcome.row[i] && outcome.row[i].userid && outcome.row[i].itemid) {
                ...
                if (this.report.isediting) {
                    ...
                    if (!this.current || info.cell.get('id') != this.current.node.get('id')) {
                        ...
                        if (!this.current.has_changed() || confirm(M.str.gradereport_marking.ajaxfieldchanged)) {
 ...
 * @param {Y.Node} cell
 */
M.gradereport_marking.classes.ajax.prototype.display_submission_error = function(message, cell) {
    var erroroverlay = new this.report.Y.Overlay({
        headerContent : '<div><strong class="error">'+M.str.gradereport_marking.ajaxerror+'</strong>  <em>'+M.str.gradereport_marking.ajaxclicktoclose+'</em></div>',
        bodyContent : message,
        visible : false,
        zIndex : 3
    });
 ...
 * @param {Int} itemid
 */
M.gradereport_marking.classes.existingfield = function(report, userid, itemid) {
    this.report = report;
    this.userid = userid;
    this.itemid = itemid;
    this.editfeedback = M.gradereport_marking.ajax.showquickfeedback;
 ...
 * via prototyping
 */
M.gradereport_marking.classes.existingfield.prototype.userid = null;
M.gradereport_marking.classes.existingfield.prototype.itemid = null;
M.gradereport_marking.classes.existingfield.prototype.editfeedback = false;
M.gradereport_marking.classes.existingfield.prototype.grade = null;
M.gradereport_marking.classes.existingfield.prototype.oldgrade = null;
M.gradereport_marking.classes.existingfield.prototype.keyevents = [];
/**
 * Handles saving of changed on keypress
 *
 * @function
 * @this {M.gradereport_marking.classes.existingfield}
 * @param {Event} e
 */
M.gradereport_marking.classes.existingfield.prototype.keypress_enter = function(e) {
    this.submit();
};
/**
 ...
 */
M.gradereport_marking.classes.existingfield.prototype.keypress_tab = function(e, ignoreshift) {
    e.preventDefault();
    var next = null;
    if (e.shiftKey) {
        if (ignoreshift) {
 ...
 * @param {Event} e
 */
M.gradereport_marking.classes.existingfield.prototype.keypress_arrows = function(e) {
    var next = null;
    switch (e.keyCode) {
        case 37:    // Left
 ...
 * @param {Y.Node} node
 */
M.gradereport_marking.classes.existingfield.prototype.move_focus = function(node) {
    if (node) {
 ...
 * @return {Bool}
 */
M.gradereport_marking.classes.existingfield.prototype.has_changed = function() {
    if (this.editfeedback) {
 ...
 * @this {M.gradereport_marking.classes.existingfield}
 */
M.gradereport_marking.classes.existingfield.prototype.submit = function() {
    if (!this.has_changed()) {
        return;
    }
 ...
 * @class textfield
 * @constructor
 * @this {M.gradereport_marking.classes.textfield}
 * @param {M.gradereport_marking.classes.report} report
 * @param {Y.Node} node
 */
M.gradereport_marking.classes.textfield = function(report, node) {
    this.report = report;
    this.node = node;
 ...
 */
M.gradereport_marking.classes.textfield.prototype.keyevents = [];
M.gradereport_marking.classes.textfield.prototype.editable = false;
M.gradereport_marking.classes.textfield.prototype.gradetype = null;
M.gradereport_marking.classes.textfield.prototype.grade = null;
M.gradereport_marking.classes.textfield.prototype.report = null;
M.gradereport_marking.classes.textfield.prototype.node = null;
M.gradereport_marking.classes.textfield.prototype.gradespam = null;
M.gradereport_marking.classes.textfield.prototype.inputdiv = null;
M.gradereport_marking.classes.textfield.prototype.editfeedback = false;
/**
 * Replaces the cell contents with the controls to enable editing
 *
 * @function
 * @this {M.gradereport_marking.classes.textfield}
 * @return {M.gradereport_marking.classes.textfield}
 */
M.gradereport_marking.classes.textfield.prototype.replace = function() {
 ...
 * @return {Object}
 */
M.gradereport_marking.classes.textfield.prototype.commit = function() {
 ...
 * @this {M.gradereport_marking.classes.textfield}
 */
M.gradereport_marking.classes.textfield.prototype.revert = function() {
 ...
 * @return {Mixed}
 */
M.gradereport_marking.classes.textfield.prototype.get_grade = function() {
    if (this.editable) {
 ...
 * @param {Mixed} value
 */
M.gradereport_marking.classes.textfield.prototype.set_grade = function(value) {
    if (!this.editable) {
        if (value == '-') {
            value = '';
        }
 ...
 * @return {String}
 */
M.gradereport_marking.classes.textfield.prototype.get_feedback = function() {
    if (this.editable) {
 ...
 * @param {Mixed} value
 */
M.gradereport_marking.classes.textfield.prototype.set_feedback = function(value) {
    if (!this.editable) {
 ...
 * @return {Bool}
 */
M.gradereport_marking.classes.textfield.prototype.has_changed = function() {
 ...
 * @this {M.gradereport_marking.classes.textfield}
 */
M.gradereport_marking.classes.textfield.prototype.attach_key_events = function() {
 ...
 * @class scalefield
 * @constructor
 * @inherits M.gradereport_marking.classes.textfield
 * @base M.gradereport_marking.classes.textfield
 * @this {M.gradereport_marking.classes.scalefield}
 * @param {M.gradereport_marking.classes.report} report
 * @param {Y.Node} node
 */
M.gradereport_marking.classes.scalefield = function(report, node) {
    this.report = report;
    this.node = node;
    this.gradespan = node.one('.gradevalue');
    this.inputdiv = this.report.Y.Node.create('<div></div>');
    this.editfeedback = this.report.ajax.showquickfeedback;
    this.grade = this.report.Y.Node.create('<select type="text" class="text" /><option value="-1">'+M.str.gradereport_marking.ajaxchoosescale+'</option></select>');
 ...
 */
M.gradereport_marking.classes.scalefield.prototype.scale = [];
/**
 ...
 */
M.gradereport_marking.classes.scalefield.prototype.get_grade = function(){
    if (this.editable) {
 ...
 * @param {String} value
 */
M.gradereport_marking.classes.scalefield.prototype.set_grade = function(value) {
    if (!this.editable) {
        if (value == '-') {
            value = '-1';
        }
 ...
 * @return {Bool}
 */
M.gradereport_marking.classes.scalefield.prototype.has_changed = function() {
    if (!this.editable) {
        return false;
    }
 ...
 */
for (var i in M.gradereport_marking.classes.textfield.prototype) {
    if (!M.gradereport_marking.classes.scalefield.prototype[i]) {
        M.gradereport_marking.classes.scalefield.prototype[i] = M.gradereport_marking.classes.textfield.prototype[i];
    }
}