...
require_once $CFG->dirroot.'/grade/report/marking/lib.php';
...
$PAGE->set_url(new moodle_url('/grade/report/marking/index.php', array('id'=>$courseid)));
...
require_capability('gradereport/marking:view', $context);
require_capability('moodle/grade:viewall', $context);

/// return tracking object
$gpr = new grade_plugin_return(array('type'=>'report', 'plugin'=>'marking', 'courseid'=>$courseid, 'page'=>$page));

/// last selected report session tracking
if (!isset($USER->grade_last_report)) {
    $USER->grade_last_report = array();
}
$USER->grade_last_report[$course->id] = 'marking';
...
if (!empty($target) && !empty($action) && confirm_sesskey()) {
    grade_report_marking::process_action($target, $action);
}

$reportname = get_string('pluginname', 'gradereport_marking');

/// Print header
print_grade_page_head($COURSE->id, 'report', 'marking', $reportname, false, $buttons);

//Initialise the marking report object that produces the table
//the class grade_report_grader_ajax was removed as part of MDL-21562
$report = new grade_report_marking($courseid, $gpr, $context, $page, $sortitemid);
